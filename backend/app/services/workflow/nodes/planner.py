...
def _build_planner_prompt(payload: SafePlannerPayload) -> str:
    ...
    return (
        ...
        "- World/craft rules are in bible_context above (hard); follow and do not contradict.\n"
        ...
        "## Output requirements\n"
        "- Revision rule: If prior attempt data exists, patch only the feedback-flagged parts. Do not rewrite the whole spine. Ensure new causal advancement; never repackage previous chapter events as 'new'."
        "- Each ground_truth_events row must encode a new state change for this chapter—not a recap masquerading as events.\n"
        "- Event granularity (hard): Merge continuous physical actions/dialogue into macro EVENTs." 
        "Crucially, treat major psychological shifts, internal struggles, or epiphanies as distinct, valid EVENTs. Split events when physical goals, dominant actors, scene location, or internal psychological states change.\n"
        ...
        "- must_include_beats: 3–5 concrete narrative beats. At least one beat must focus on the POV character's internal psychological processing, emotional residue, or decision-making.\n"
        ...
        "- ending_state_shift: one sentence on how end-state must differ from opening state. (can be a physical plot shift OR a major psychological/emotional realization)\n"
        ...
        "- Spatial/Boundary consistency: All proposed_new_nodes and beats MUST be physically plausible BEFORE the ending_boundary_rule. Never force the Author to write interactions that belong in the next chapter. Defer out-of-bounds entities to the future.\n"
        ...
        "- author_safe_continuity_notes: 0–4 short lines of continuity reminders. Translate hidden graph facts/unresolved threads into surface-level suspense or sensory clues the POV can feel. ZERO unearned spoilers or unapproved proper names."
    )
